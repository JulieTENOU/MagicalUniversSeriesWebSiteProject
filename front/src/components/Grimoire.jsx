...
export default function CustomizedDialogs(data) {
  ...
  const handleSave = () => {
    const data = {
      autres_inventory: autres,
    };
    console.log(data);
    fetch(`/inventories/api/updateInventory/${character.ID_character}`,{
      method: "PUT",
      headers:{
  ...
  return (
    <div>
      ...
      <Dialog
        onClose={handleClose}
        open={open}
        PaperProps={{style:{minWidth:'70vw', minHeight:'70vh', backgroundColor:'beige'},}}
      >
        ...
        <DialogContent>
          ...
          <Typography gutterBottom>
            Exemples d'utilisations magiques <button onClick={() => {setSimple(true); setCombinaison(false); setNotes(false);} }>simples</button> / <button onClick={() => {setSimple(false); setCombinaison(true); setNotes(false);} }>combinées</button> / <button onClick={() => {setSimple(false); setCombinaison(false); setNotes(true);} }>Mes notes</button>
          </Typography>
          {simple ?  <Box>
            <Typography gutterBottom>
            <table>
          ...
          <Typography gutterBottom>
              ...
              <TextField id="newNotes" multiline sx={{minHeight:'150px', minWidth:'150px'}} onChange={(e) => setText(e.target.value)} inputProps={{style:{ height:"150px"}}}/>
              <Button onClick={() => {
                autres += text;
                console.log(autres);
                handleSave();
